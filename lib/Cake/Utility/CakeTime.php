...
/**
 * The format to use when formatting a time using `TimeHelper::timeAgoInWords()`
 * and the difference is more than `TimeHelper::$wordEnd`
 *
 * @var string
...
/**
 * The format to use when formatting a time using `TimeHelper::timeAgoInWords()`
 * and the difference is less than `TimeHelper::$wordEnd`
 *
 * @var array
 * @see TimeHelper::timeAgoInWords()
 */
 ...
 */
	public static function niceShort($dateString = null, $userOffset = null) {
		...
		$y = self::isThisYear($date) ? '' : ' %Y';
                ...
                $d = self::format('w', $dateString, false, $userOffset);
		...
		if (self::isToday($dateString, $userOffset)) {
			...
			$ret = __d('cake', 'Yesterday, %s', self::_strftime("%H:%M", $date));
                } elseif (self::isTomorrow($dateString, $userOffset)) {
			$ret = __d('cake', 'Tomorrow, %s', self::_strftime("%H:%M", $date));
                } elseif (self::wasWithinLast('7 days', $dateString, $userOffset)) {
			$ret = sprintf('%s, %s', $day[$d], self::_strftime("%H:%M", $date));
                } elseif (self::isWithinNext('7 days', $dateString, $userOffset)) {
 ...
 */
	public static function timeAgoInWords($dateTime, $options = array()) {
		...
		if (is_array($options) && isset($options['accuracy'])) {
			...
			$accuracy = self::$wordAccuracy;
                ...
                if (is_array($options) && isset($options['element'])) {
		...
		if ($diff > abs($now - self::fromString($end))) {
			...
			if (!$backwards) {
                ...
                if (self::wasWithinLast('7 days', $dateTime, $userOffset) || self::isWithinNext('7 days', $dateTime, $userOffset)) {
                    $relativeDate = self::niceShort($dateTime , $userOffset);
                }
 ...
 * @link http://book.cakephp.org/2.0/en/core-libraries/helpers/time.html#testing-time
 */
	public static function isWithinNext($timeInterval, $dateString, $userOffset = null) {
		$tmp = str_replace(' ', '', $timeInterval);
		if (is_numeric($tmp)) {
			$timeInterval = $tmp . ' ' . __d('cake', 'days');
		}
